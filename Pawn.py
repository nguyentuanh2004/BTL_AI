...
class Pawn(Piece):
	def __init__(self, x, y, color, board):
		...
		#img_path = f'images/pinkpawn.png'
		self.img = pygame.image.load(img_path)
		self.img = pygame.transform.scale(self.img, (board.tile_width, board.tile_height))
		self.notation = 'p'

	def __deepcopy__(self, memo):
		"""Create a deep copy of the Pawn object."""
		id_self = id(self)
		if id_self in memo:
			return memo[id_self]
			
		# Use the parent class's __deepcopy__ to handle basic attributes
		result = super().__deepcopy__(memo)
		
		# No need to handle img separately as it's already handled in the parent's __deepcopy__
